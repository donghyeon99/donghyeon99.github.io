...
한국연구재단(NRF)의 "불법드론 무력화를 위한 인공지능 기반의 협력적 자율주행 스웜 안티드론 시스템 개발" 과제에서 멀티 UAV 편대 제어(Multi-UAV Formation Control)를 연구했다. 다수의 무인항공기(UAV)가 중앙 통신 없이 스스로 포메이션을 유지하는 분산 제어기를 개발해, 1대의 Virtual Leader(실기체가 아닌 가상 기준점)와 4대의 팔로워 구성에서 **포메이션 오차 ±0.2m 이하**를 확인했다. S자형 경로 추종 실험에서는 기존 알고리즘 대비 **위치 오차(MSE)를 약 61%** 줄였다.

## 문제

안티드론 스웜은 여러 대의 UAV가 팀을 이뤄 불법드론에 협력적으로 대응하는 시스템이다. 어려움은 다수의 기체가 중앙 통신 없이, 이웃 기체와의 상대 벡터·거리 정보만으로 포메이션을 유지해야 한다는 데 있다. 비행이 길어질수록 센서 오차가 쌓이고, 기체 동역학의 비선형성까지 겹친다. 포메이션 유지와 충돌 회피를 동시에 만족하는 제어기가 필요했다.

## 역할

고려대학교 Advanced Control System Lab 석사과정에서 참여한 과제다. Multi-UAV 협력 제어·충돌 회피 알고리즘 개발과 검증용 하드웨어 제작을 맡았다.

## 핵심 기여

- 최근 일정 구간(window)의 데이터만 사용하는 Finite-Memory(유한 메모리) 방식으로 분산 제어기를 설계해, 오차 누적과 비선형성이 제어 성능을 무너뜨리지 않도록 억제.
- 팔로워 간의 연결 관계(상대 거리·각도)를 그래프 기반 상호위상 모델(Graph Model)로 협력 항법에 반영해 동적 포메이션의 안정성 확보.
- 팔로워들이 실기체 리더가 아닌 공통의 가상 목표점(Virtual Leader)을 추종하는 구조로, 중앙 통신 없는 분산 협력 제어 구현.
...
알고리즘은 MATLAB 시뮬레이션으로 먼저 검증했다. 장애물 회피와 Virtual Leader 추종 포메이션 유지가 의도대로 동작하는 것을 확인한 뒤 실기체 실험으로 넘어갔다.
...
An anti-drone swarm is a system in which multiple UAVs team up to respond cooperatively to illegal drones. The hard part is that the vehicles must hold formation without central communication, using only relative vectors and distances to their neighbors. Sensor error accumulates as flights get longer, on top of nonlinear vehicle dynamics. The controller had to satisfy formation keeping and collision avoidance at the same time.
...
- Designed a distributed controller on a Finite-Memory scheme that uses only data from a recent window, which limits how much accumulated error and nonlinearity can degrade control.
- Modeled the inter-follower connectivity (relative distance and angle) as a graph-based interaction topology (Graph Model) and used it in cooperative navigation to keep dynamic formations stable.
- Had Followers track a common virtual target point (Virtual Leader) instead of a physical leader vehicle, so cooperative control needs no central communication.
- Developed a collision-avoidance algorithm for obstacles during formation flight and verified it in MATLAB simulation.
- Built hexacopters based on Pixhawk (an open-source flight controller) with UWB for inter-vehicle distance measurement and an IMU, and validated the algorithms on real vehicles in a ROS environment.
