...
실기체 검증은 UWB·IMU·GPS와 Mini PC를 탑재한 Pixhawk 기반 헥사콥터로 진행. Virtual Leader를 추종하는 팔로워 4대의 편대 비행에서 포메이션 오차 **±0.2m 이하**와 빠른 수렴 특성 확인.
...
An anti-drone swarm is a system in which multiple UAVs team up to respond cooperatively to illegal drones. The hard part is that the vehicles must hold formation without central communication, with only relative vectors and distances to their neighbors as input. Sensor error accumulates as flights get longer, on top of nonlinear vehicle dynamics. The controller had to satisfy formation keeping and collision avoidance at the same time.
...
I ran real-vehicle validation with Pixhawk-based hexacopters carrying UWB, IMU, GPS, and a Mini PC. In formation flight of four Followers tracking a Virtual Leader, formation error stayed **within ±0.2 m** with fast convergence.
